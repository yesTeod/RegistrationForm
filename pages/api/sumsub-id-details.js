...
export default async function handler(req, res) {
  ...
  try {
    ...
    const endpoint = `/resources/applicants/${encodeURIComponent(applicantId)}/one`;
    const applicantData = await makeRequest('GET', endpoint);
    ...
    const extractedData = {
      ...
      status: applicantData.review?.reviewStatus || 'pending'
    };
    
    return res.status(200).json(extractedData);
  } catch (error) {
    console.error('ID data fetch error:', error);
    return res.status(500).json({ error: error.message || 'Failed to fetch identification data' });
  }
}